...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (eventType === "orders/create") {
      ...
      const name = customer.first_name || (order.billing_address && order.billing_address.first_name) || "";
    ...
    if (eventType === "checkouts/update") {
      ...
      const phoneRaw = checkout.phone || (checkout.customer && checkout.customer.phone) || "";
      const phone = phoneRaw.replace(/^\+/, "");
